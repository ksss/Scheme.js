...
pp = function (obj) {
	if (DEBUG) {
		...
		for (var i = 0, it; (it = args[i]); i++) {
			console.log("# pp", util.inspect(it, true, 5));
		}
		console.log();
	}
};
...
(function () {
	...
	Scheme.prototype.perse = function (src) {
		src = src || this.src
		return Wark.run(src, function (w) {
			var string, value, cons;
			string = function () {
				var string = '';
				while (w.ch) {
					if (w.ch !== ' ' && w.ch !== ')') {
	...
	Scheme.prototype.builtin = function () {
		...
		this.define('lambda', function (args) {
			return function (a, b) {return a.car * a.car};
		});
